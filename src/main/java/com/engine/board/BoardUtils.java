...
public class BoardUtils {
    ...
    public static boolean isKingPawnTrap(final Board board, final King king, final int frontTileNumber) {
        final Piece piece = board.getPiece(frontTileNumber);

        if (piece == null) {
            return false;
        }

        final Piece.PieceType frontTilePieceType = piece.getPieceType();
        final Alliance frontTilePieceAlliance = piece.getPieceAlliance();

        return frontTilePieceType == Piece.PieceType.PAWN && frontTilePieceAlliance != king.getPieceAlliance();
    }
